...
const createModel = async function (server, connection, name, path) {
    ...
    if (typeof file === 'object' &&
        ...
        return connection.model(name, file);
    }

    throw new Error(`unable to create model for file: ${path}`);
};
...
const connect = async function (server, connections) {
    ...
    for (const options of connections) {
        ...
        for (const path of files) {
            const name = Capitalize(Path.parse(path).name);
            models[name] = await createModel(server, connection, name, path);
            server.log(
                ['info', 'hapi-mongoose2'],
